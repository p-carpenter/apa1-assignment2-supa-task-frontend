...
describe('useForm hook', () => {
  ...
  it('should handle input changes correctly', () => {
    ...
    act(() => {
      result.current.handleChange(
        createChangeEvent('name', 'John Doe')
      );
    });
  ...
  it('should handle checkbox input correctly', () => {
    ...
    act(() => {
      result.current.handleChange(
        createChangeEvent('terms', true, 'checkbox')
      );
    });
  ...
  it('should validate fields on blur', () => {
    ...
    expect(mockValidateFn).toHaveBeenCalled();
    expect(result.current.formErrors.name).toBe('Name must be at least 3 characters long');
  });
  ...
  it('should work with fileState option', () => {
    ...
    const { result } = renderHook(() => 
      useForm(initialValues, mockValidateFn, mockSubmit, { fileState: mockFileState })
    );
