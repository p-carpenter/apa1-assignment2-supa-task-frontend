...
const ArchiveCatalog = ({ setContextMenu }) => {
  const { incidents, incidentsByDecade, setDisplayedIncident, setCurrentIncidentIndex } = useIncidents();
  ...
  const getIncidentYear = (incident) => {
    ...
    try {
      ...
      console.error("Error extracting year from incident date:", incident.incident_date);
      return null;
    }
  };

  // Extract individual years from incidents safely
  useEffect(() => {
  ...
  const filteredIncidents = incidents.filter((incident) => {
    ...
    const matchesYear = activeYear === "all" || 
      (year && year.toString() === activeYear);

    // Handle search matching
    const matchesSearch =
      searchQuery === "" ||
      (incident.name && incident.name.toLowerCase().includes(searchQuery.toLowerCase())) ||
        ...
        <div className="console-window">
          ...
          <div className="console-content">
            ...
            <div className="console-section catalog-filters">
              <div className="command-line">
                <span className="prompt">user@archive:~$</span>
                <span className="command">filters --define</span>
              </div>
            ...
            <div className="console-section">
              <div className="command-line">
                <span className="prompt">user@archive:~$</span>
                <span className="command">display --results --format=grid</span>
              </div>
                  ...
                  {filteredIncidents.map((incident) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="incident-name">{incident.name || 'Unknown Incident'}</div>
