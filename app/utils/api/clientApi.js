 ...
 * @throws {Error} Processed API error with standardised format
 */
 ...
 */
const validateRequest = (path, method) => {
  if (!path) {
 ...
 */
const handleResponse = async (response, path, method) => {
  if (!response.ok) {
 ...
 * @param {string} path - API endpoint path
 * @param {string} method - HTTP method
 * @returns {Promise<e>} Error object with additional properties
 */
const createErrorFromResponse = async (response, path, method) => {
  let errorData;
  try {
 ...
 */
const handleError = (error, path, method) => {
  if (!error.path) {
    error.path = path;
    error.method = method;
  }
  ...
  if (
    ...
    error.details = "Network connection error detected";
  }
