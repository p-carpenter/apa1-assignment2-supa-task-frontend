...
export const handleApiError = (error, options = {}) => {
  ...
  // First, log the error (can be customized or disabled)
  if (logger) {
...
/**
 * Determines if user should be redirected to login based on error type
 * 
 * @param {string} errorType - The type of error
 * @returns {boolean} Whether user should be redirected to login
 */
export const shouldRedirectToLogin = (errorType) => {
  return [
...
export const isValidError = (error) => {
  return Boolean(
    error && 
    (hasErrorMessage(error.message) || error.type)
  );
};
...
export const resolveError = (externalError, internalError, errorMessage) => {
  ...
  // If no valid external error, check internal error
  if (isValidError(internalError)) {
    return internalError;
  }
  
  // If no valid error objects, check error message
...
/**
 * Create a standardized error object for common error types
 * 
 ...
 */
export const createError = (type, customMessage = null, status = null) => {
  return {
    type: type,
...
const getDefaultStatusForType = (type) => {
  // Reverse lookup in STATUS_CODE_MAP
  for (const [code, errorType] of Object.entries(STATUS_CODE_MAP)) {
    if (errorType === type) {
      return parseInt(code, 10);
    }
  }
  
  // Default status codes for types not in the map
  switch (type) {
