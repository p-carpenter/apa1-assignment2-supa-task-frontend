...
export const withAuth = (handler) => async (req) => {
  try {
    const { accessToken, refreshToken, cookieString } =
    ...
    const authenticatedRequest = attachAuthToRequest(
      req,
      accessToken,
      refreshToken,
      cookieString
 ...
 */
const createAuthenticationErrorResponse = () => {
  return new Response(
    JSON.stringify({
      message:
 ...
 */
const attachAuthToRequest = (req, accessToken, refreshToken, cookieString) => {
  req.auth = {
    accessToken,
    refreshToken,
 ...
 */
const createAuthMiddlewareErrorResponse = (error) => {
